...
| Single validated Raspberry | The demo is validated on one Raspberry Pi setup, which now includes a Waveshare 5" Touch Screen. | If that device, display, microphone, or speaker fails, there is no identical hardware backup. | Web voice and text fallback are documented in the demo checklist. |
